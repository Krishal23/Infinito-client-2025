...
function Auth() {
  ...
  const sendOtp = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axiosInstance.post('/auth/signup/send-otp', {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { accessToken, user } = res.data;
      localStorage.setItem('token', accessToken);
      axiosInstance.defaults.headers.common['Authorization'] = `Bearer ${accessToken}`; // set for future requests
